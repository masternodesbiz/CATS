...
{
    if (nHeight >        0) return     400000 * COIN;
...
CAmount CMasternode::GetBlockValue(int nHeight)
{
    ...
    if (nHeight >   2000000) return     0.0 * COIN;
    if (nHeight >   1500000) return     10 * COIN;
    if (nHeight >   1000000) return     25  * COIN;
    if (nHeight >    500000) return     50 * COIN;
    if (nHeight >         2) return     100 * COIN;
    if (nHeight >         1) return     195000000 * COIN;
    if (nHeight >         0) return     0.5 * COIN;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    if (mi != mapBlockIndex.end() && (*mi).second) {
        ...
        int nConfHeight = pMNIndex->nHeight + MASTERNODE_MIN_CONFIRMATIONS - 1; // block for 1000 CATS tx -> 1 confirmation

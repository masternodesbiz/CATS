...
UniValue getgenerate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "It is set with the command line argument -gen (or CATS.conf setting gen)\n"
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    //     throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "CATS is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "CATS is downloading blocks...");

...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected CATS and zCATS supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zCATSsupply']:
                assert_equal(node['zCATSsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check CATS and zCATS supply at the beginning
        # ------------------------------------------
        # zCATS supply: 2 coins for each denomination
        ...
        # CATS supply: block rewards minus burned fees for minting
        ...
        # Verify that CATS and zCATS supplies were properly updated after the spends and reorgs
        self.log.info("Check CATS and zCATS supply...")

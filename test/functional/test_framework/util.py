...
def assert_fee_amount(fee, tx_size, fee_per_kB):
    ...
    if fee < target_fee:
        raise AssertionError("Fee of %s CATS too low! (Should be %s CATS)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    if fee > (tx_size + 20) * fee_per_kB / 1000:
        raise AssertionError("Fee of %s CATS too high! (Should be %s CATS)" % (str(fee), str(target_fee)))
...
def initialize_datadir(dirname, n):
    ...
    with open(os.path.join(datadir, "CATS.conf"), 'w', encoding='utf8') as f:
        f.write("regtest=1\n")
...
def get_auth_cookie(datadir):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "CATS.conf")):
        with open(os.path.join(datadir, "CATS.conf"), 'r', encoding='utf8') as f:
            for line in f:
